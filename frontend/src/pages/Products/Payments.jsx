...
const CartItem = ({ item, onUpdateQuantity, onRemove }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (value === '' || (!isNaN(value) && value >= 1 && value <= item.quantity)) {
      setInputQuantity(value);
    }
  };

  const handleBlur = () => {
    let newQuantity = Number(inputQuantity);
    if (inputQuantity === '') {
      newQuantity = item.cartQuantity;
    }

    if (newQuantity > item.quantity) {
      alert('Not enough items available in stock');
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="mt-6">
          ...
          <button 
            ...
            className="bg-amber-900 text-white px-6 py-2 rounded hover:bg-amber-800 transition-colors duration-300 mr-4"
          >
            Proceed to Checkout
          </button>
          <button 
